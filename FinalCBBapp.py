...
# ========== UPDATED FUNCTION - CHANGES HIGHLIGHTED BELOW ==========
def create_editable_summary_table(team_rank, replacement_value):
    ...
    # ⭐ NEW: Initialize session state for edited values if not exists
    # This ensures values persist between tab switches and match the Team Dashboard
    if 'initialized_vws' not in st.session_state or st.session_state.get('last_team_rank') != team_rank or st.session_state.get('last_replacement_value') != replacement_value:
        st.session_state.initialized_vws = True
        st.session_state.last_team_rank = team_rank
        st.session_state.last_replacement_value = replacement_value
        for i in range(10):
            st.session_state[f'vws_edit_{i+1}'] = float(vws_values[i])
    ...
    for i in range(5):
        with cols[i]:
            # ⭐ CHANGED: Now uses session state value instead of vws_values[i]
            edited_value = st.number_input(
                f"Player Rank {i+1}",
                min_value=0.0,
                max_value=30.0,
                value=float(st.session_state.get(f'vws_edit_{i+1}', vws_values[i])),  # ⭐ CHANGED
                step=0.1,
                format="%.2f",
                key=f"vws_edit_{i+1}"
            )
